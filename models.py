...
class invariant_encoder(nn.Module):
    def __init__(self,config,domain:int,use_cuda = False):
        ...
        self.bn_momentum = config['bn_momentum']
        self.use_cuda =use_cuda
        self.vae_individual_shared_encoder = nn.Sequential(
...
class CMR_NET(nn.Module):
    def __init__(self, config: dict, use_cuda=False):
        ...
        # Initialize all sub-networks as class attributes
        self.qzx_extractor = specific_encoder(config, 0, use_cuda)  # for text
        self.qzy_extractor = specific_encoder(config, 1, use_cuda)  # for image
        self.rzx_extractor = invariant_encoder(config, 0, use_cuda)
        self.rzy_extractor = invariant_encoder(config, 1, use_cuda)
        self.decode_x = feature_decoder(config, 0, use_cuda)
        self.decode_y = feature_decoder(config, 1, use_cuda)
        self.c_extractor = c_encoder(config, use_cuda)
        self.gengrator = distribution_gengrator(config, use_cuda)
        self.c_recon = c_decoder(config, use_cuda)
        
        # Initialize hyperparameters
        self.ALPHA = config.get('alpha', 1.0)
        self.BETA = config.get('beta', 1.0)
        self.GAMMA = config.get('gamma', 1.0)
        self.LAMBDA = config.get('lambda', 1.0)
        self.ar = config.get('ar', 1000.0)
    ...
    # added to match main.py you can delete this later
    # In your CMR_NET class, add these methods:
    def text_encoder(self, x):
        ...
        return torch.cat([qzx_mean, rzx_mean], dim=1)

    def image_encoder(self, y):
        qzy_mean, qzy_logvar = self.qzy_extractor(y)
        rzy_mean, rzy_logvar = self.rzy_extractor(y)
        # Return the combined features
        return torch.cat([qzy_mean, rzy_mean], dim=1)
    ...
    def forward(self, x, y, label, step):
        ...
        if self.use_cuda:
            ...
            qzx_logvar = torch.min(qzx_logvar, torch.Tensor([9.21035]))
            qzy_logvar = torch.min(qzy_logvar, torch.Tensor([9.21035]))
            rzx_logvar = torch.min(rzx_logvar, torch.Tensor([9.21035]))
            rzy_logvar = torch.min(rzy_logvar, torch.Tensor([9.21035]))
        ...
        reg_coeff = torch.Tensor(1.0 - torch.exp(-torch.Tensor([step]).float() / self.ar))
        if self.use_cuda:
            reg_coeff = reg_coeff.cuda()
        reg_coeff = reg_coeff.detach()

        joint_loss = x_recon_loss + y_recon_loss \
                     + self.ALPHA * reg_coeff * kl_x_loss \
                     + self.ALPHA * reg_coeff * kl_y_loss \
                     + self.BETA * reg_coeff * aligin_x_loss \
                     + self.BETA * reg_coeff * aligin_y_loss \
